  ...
  if (!canSeeLogs) {
    return (
      <section className="rounded-2xl bg-white p-6 shadow-sm">
        ...
        <p className="mt-2 text-slate-500">Seu perfil pode visualizar status das integrações, mas não tem acesso aos logs técnicos.</p>
      </section>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <header className="rounded-2xl bg-white p-6 shadow-sm">
        <p className="text-xs font-bold uppercase text-slate-500">Auditoria</p>
        <h1 className="text-3xl font-black">Logs de integração</h1>
        <p className="mt-2 text-slate-500">Eventos inbound, outbound, testes mockados e erros controlados.</p>
      </header>
            ...
            <tbody>
              {logs.map((log) => (
                <tr key={log.id} className="border-t border-slate-100 align-top">
                  ...
                  <td><span className="rounded-full bg-slate-100 px-2 py-1 text-xs font-black">{log.status}</span></td>
                  <td>{log.external_id ?? log.payload.externalId ?? "-"}</td>
                  <td className="max-w-[220px] text-red-700">{log.error_message ?? log.message ?? "-"}</td>
                  <td>
                    <details>
                      <summary className="cursor-pointer font-bold text-red-600">Abrir</summary>
              ...
              {!logs.length && <tr><td colSpan={8} className="p-6 text-center text-slate-500">Nenhum log encontrado.</td></tr>}
            </tbody>
          </table>
        </div>
      </section>
