  ...
  return (
    <div className="space-y-6">
      ...
      <header className="rounded-2xl bg-white p-6 shadow-sm">
        <p className="text-xs font-bold uppercase text-slate-500">API externa</p>
        <h1 className="text-3xl font-black">Webhooks / API</h1>
        <p className="mt-2 max-w-3xl text-slate-500">Receba pedidos de sistemas externos por um webhook genérico. O token deve ser enviado no header Authorization: Bearer TOKEN ou x-webhook-secret.</p>
      </header>
      ...
      <form action={saveIntegration} className="rounded-2xl bg-white p-5 shadow-sm">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <label className="flex items-center gap-2 rounded-xl border border-slate-200 p-3 font-bold"><input name="enabled" type="checkbox" defaultChecked={Boolean(integration.is_enabled ?? integration.enabled)} disabled={!canEdit} /> Webhook ativo</label>
          <label className="flex items-center gap-2 rounded-xl border border-slate-200 p-3 font-bold"><input name="receive_orders" type="checkbox" defaultChecked={settings.receiveOrders ?? true} disabled={!canEdit} /> Permitir criação de pedidos</label>
          <label className="flex items-center gap-2 rounded-xl border border-slate-200 p-3 font-bold"><input name="send_order_status" type="checkbox" defaultChecked={settings.sendOrderStatus ?? false} disabled={!canEdit} /> Permitir atualização de status</label>
        </div>
        <div className="mt-5 rounded-xl bg-slate-50 p-4">
          <p className="text-sm font-bold text-slate-500">URL do webhook</p>
