    ...
    <div className="space-y-6">
      ...
      <section className="rounded-2xl bg-white p-6 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <h1 className="text-3xl font-black text-[#1b1a17]">Cupons e fidelidade</h1>
            <p className="mt-1 text-[#9c988f]">Crie descontos e regras simples para incentivar recompra no cardápio online.</p>
          </div>
          {canEdit && (
            <details className="group relative">
              <summary className="inline-flex cursor-pointer list-none items-center justify-center rounded-xl bg-gradient-to-r from-[#c5362e] to-[#FF2A35] px-6 py-3 text-sm font-black text-slate-950 shadow-[0_14px_30px_rgba(229,9,20,0.18)]">
                Adicionar cupom
              </summary>
              <form action={saveCoupon} className="absolute right-0 z-10 mt-3 w-[min(92vw,520px)] rounded-2xl border border-[#e7e4dd] bg-white p-5 shadow-2xl">
                <h2 className="text-xl font-black">Novo cupom</h2>
                <p className="mt-1 text-sm text-[#9c988f]">Defina código, valor do desconto e regras de uso.</p>
                <div className="mt-4 grid gap-3">
                  ...
                  <div className="grid gap-3 sm:grid-cols-2">
                    <label className="text-sm font-bold text-[#9c988f]">Início<input className="field-light mt-1" name="starts_at" type="datetime-local" /></label>
                    <label className="text-sm font-bold text-[#9c988f]">Fim<input className="field-light mt-1" name="ends_at" type="datetime-local" /></label>
                  </div>
        ...
        <div className="mt-6 overflow-hidden rounded-xl border border-[#e7e4dd]">
          <div className="grid grid-cols-[1.4fr_120px_150px_120px_120px_160px] gap-3 border-b border-[#e7e4dd] bg-[#faf9f6]/80 px-4 py-3 text-[11px] font-black uppercase text-[#9c988f] max-xl:hidden">
            <span>Código</span>
            <span>Desconto</span>
          ...
          <div className="divide-y divide-[#e7e4dd]">
            {((coupons ?? []) as Coupon[]).map((coupon) => (
              <div key={coupon.id} className="grid gap-3 px-4 py-3 transition hover:bg-[#f6ece9]/50 xl:grid-cols-[1.4fr_120px_150px_120px_120px_160px] xl:items-center">
                <div className="min-w-0">
                  <p className="truncate text-base font-black text-[#1b1a17]">{coupon.code}</p>
                  <p className="truncate text-sm text-[#9c988f]">{coupon.description || "Sem descrição"}</p>
                  <p className="mt-1 text-xs text-[#b0aaa0]">Válido: {dateLabel(coupon.starts_at)} até {dateLabel(coupon.ends_at)}</p>
                </div>
                ...
                <span className={coupon.active ? "inline-flex h-9 items-center justify-center rounded-full bg-emerald-50 px-4 text-xs font-black text-emerald-700" : "inline-flex h-9 items-center justify-center rounded-full bg-[#f1efea] px-4 text-xs font-black text-[#9c988f]"}>
                ...
                <div className="flex justify-start gap-2 xl:justify-end">
                  {canEdit && (
                    <form action={deleteCoupon}>
                      ...
                      <button className="inline-flex h-10 items-center rounded-lg border border-[#eeccc7] px-4 text-sm font-black text-[#c5362e] transition hover:bg-[#f6ece9]">Excluir</button>
                    </form>
                  )}
                </div>
              </div>
            ...
            {!(coupons ?? []).length && (
              <div className="px-4 py-10 text-center text-sm font-semibold text-[#9c988f]">Nenhum cupom cadastrado.</div>
            )}
          </div>
        </div>
      </section>
      ...
      <section className="rounded-2xl bg-white p-6 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <h2 className="text-2xl font-black text-[#1b1a17]">Programa de fidelidade</h2>
            <p className="mt-1 text-[#9c988f]">Cada pedido pago ou concluído gera 1 ponto, desde que esteja dentro da campanha.</p>
          </div>
          <span className={currentLoyalty.enabled ? "rounded-full bg-emerald-50 px-4 py-2 text-xs font-black text-emerald-700" : "rounded-full bg-[#f1efea] px-4 py-2 text-xs font-black text-[#9c988f]"}>
            {currentLoyalty.enabled ? "Ativo" : "Inativo"}
          </span>
        </div>

        <form action={saveLoyaltyProgram} className="mt-5 grid gap-3 lg:grid-cols-5">
          <label className="flex items-center gap-2 rounded-xl border border-[#e7e4dd] bg-[#faf9f6] p-3 text-sm font-bold">
          ...
          <div className="flex h-12 items-center rounded-xl border border-[#e7e4dd] bg-[#faf9f6] px-4 text-sm font-black text-[#403d38]">
            1 ponto por pedido
          </div>
          ...
          <label className="space-y-1 lg:col-span-3">
            <span className="text-xs font-black uppercase text-[#9c988f]">Fim da campanha</span>
