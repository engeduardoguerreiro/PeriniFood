        ...
        <section key={category.id} className="rounded-2xl bg-white p-5 shadow-sm">
          ...
          <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
            {((products ?? []) as Product[]).filter((p) => p.category_id === category.id).map((product) => (
              <Link key={product.id} href={`/dashboard/products/${product.id}/edit`} className="rounded-xl border border-[#efece6] p-4 hover:border-[#c5362e]">
                <p className="font-black">{product.name}</p>
                <p className="mt-1 text-sm text-[#9c988f]">{product.description}</p>
                <p className="mt-3 font-black text-[#c5362e]">{money(product.price)}</p>
              </Link>
            ))}
          </div>
        </section>
